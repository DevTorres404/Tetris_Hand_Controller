...
# Ajustar importaciones según ubicación
try:
    from core_tetris import Motor, TETROMINOS, Pieza
    from controlador_manos import crear_controlador_manos_o_nada, ControladorMano
except ImportError:
    # Soporte para ejecución directa desde src/
    from src.core_tetris import Motor, TETROMINOS, Pieza
    from src.controlador_manos import crear_controlador_manos_o_nada, ControladorMano
...
class RenderizadorTetris:
    """
    Maneja todo el renderizado visual del juego utilizando Pygame.
    Dibuja el tablero, piezas, interfaz de usuario (HUD) y la previsualización de la cámara.
    """
    ...
    def dibujar_tablero(self, tablero: List[List[Optional[str]]]) -> None:
        """
        Dibuja el fondo y las celdas ocupadas del tablero.

        Args:
            tablero (List[List[Optional[str]]]): Matriz del tablero del juego.
        """
    ...
    def dibujar_pieza(self, pieza: Optional[Pieza]) -> None:
        """
        Dibuja la pieza actual en movimiento con efecto de brillo.

        Args:
            pieza (Pieza): La pieza activa.
        """
        if pieza is None:
            return
        margen_y = 20  # Margen superior
    ...
    def dibujar_camara(self, frame_bgr: np.ndarray) -> None:
        """
        Dibuja el frame de la cámara en la esquina superior derecha.
        Convierte de BGR (OpenCV) a RGB (Pygame) y redimensiona si es necesario.

        Args:
            frame_bgr (np.ndarray): Imagen en formato NumPy array (BGR).
        """
    ...
    def dibujar_hud(self, estado: Dict[str, Any], mano_activa: bool) -> None:
        """
        Dibuja el panel lateral con información de puntaje y controles.

        Args:
            estado (Dict): Diccionario con el estado del juego (puntaje, nivel, etc).
            mano_activa (bool): Indica si el control por gestos está habilitado.
        """
    ...
    def barrido_game_over(self, tablero: List[List[Optional[str]]]) -> None:
        """
        Efecto visual de barrido (animación) al terminar el juego.
        Va cubriendo el tablero desde abajo hacia arriba.
        """
        ...
        for y in range(FILAS - 1, -1, -1):
            for x in range(COLUMNAS):
                # Mantener la ventana responsiva durante la animación
    ...
    def menu_game_over(self, puntaje: int, lineas: int, nivel: int) -> bool:
        """
        Muestra el menú de Game Over y espera input del usuario.

        Returns:
            bool: True si el usuario decide reiniciar, False si sale.
        """
    ...
    def pantalla_carga(self, mensaje: str, tiene_mano: bool) -> None:
        """
        Muestra la pantalla de inicio/carga antes de comenzar el juego.
        """
...
class GestorAudio:
    ...
    Intenta descargar los assets automáticamente desde Google Drive si no existen localmente.
    """
    ...
    def _descargar_y_cargar_sonidos(self) -> None:
        """
        Descarga archivos .wav necesarios si faltan y los carga en pygame.mixer.
        """
        try:
...
# ============================================================
def ejecutar_juego(mano: Optional[ControladorMano] = None) -> bool:
    """
    Ejecuta una sesión completa de juego (una partida).

    Args:
        mano (Optional[ControladorMano]): Instancia del controlador de gestos, si está disponible.

    Returns:
        bool: True si el jugador quiere reiniciar al finalizar, False si quiere salir.
    """
