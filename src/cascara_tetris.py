...
class RenderizadorTetris:
    ...
    def pantalla_cargando(self):
        """Muestra pantalla de 'Cargando recursos...'."""
        cx, cy = self.pantalla.get_rect().center

        self.pantalla.fill(COLOR_FONDO_MENU)

        titulo = self.fuente_grande.render("TETRIS", True, COLOR_ACENTO)
        texto = self.fuente.render("Descargando sonidos...", True, COLOR_TEXTO_SECUNDARIO)

        self.pantalla.blit(titulo, titulo.get_rect(center=(cx, cy - 60)))
        self.pantalla.blit(texto, texto.get_rect(center=(cx, cy)))
        pygame.display.flip()
...
def ejecutar_juego(mano=None):
    ...
    # Mostrar mensaje de carga de recursos
    render.pantalla_cargando()
